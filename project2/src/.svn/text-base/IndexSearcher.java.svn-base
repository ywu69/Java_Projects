...
public class IndexSearcher {
	...
	/*
	 * Read the query files line by line and separate the words by
	 * separateWordByLine() function. Then call partialSearch function to search
	 * the queryWords in map, then get the searchResults, then store them to a
	 * LinkedHashMap
	 */
	...
	private String formatWord(String queryWord) {
		...
		queryWord = queryWord.trim().replaceAll("[^a-zA-Z0-9\\s]", "")
				.toLowerCase();
